...
import type { MemoryBucketAdapter } from '~/elements/entities/bucket/adapters/memory.bucket_adapter';
import { t } from 'nesoi/tools/joaquin/bucket';

Log.level = 'warn';


const dateBucket = t.bucket('test', $ => $
    .model($ => ({
        id: $.int,
        value: $.date,
        deep: $.obj({
...
const datetimeBucket = t.bucket('test', $ => $
    .model($ => ({
        id: $.int,
        value: $.datetime,
        deep: $.obj({
...
const decimalBucket = t.bucket('test', $ => $
    .model($ => ({
        id: $.int,
        value: $.decimal(),
        deep: $.obj({
...
const durationBucket = t.bucket('test', $ => $
    .model($ => ({
        id: $.int,
        value: $.duration,
        deep: $.obj({
    ...
    it('should store/read date properly', async () => {
        ...
        t.given(dateBucket)
            .when.bucket('test', async $ => {
                created = await $.trx.bucket('test').create.one({
                ...
                read = await $.trx.bucket('test').read.one(created.id);
                data = Object.values(($.bucket.adapter as MemoryBucketAdapter<any, any>).data);
            })


        expect(data[0].value).toBeInstanceOf(NesoiDate);
        expect(data[0].deep.value).toBeInstanceOf(NesoiDate);
        expect(data[0].deep.deeper[0].value).toBeInstanceOf(NesoiDate);
