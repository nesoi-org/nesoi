...
declare interface NesoiObj extends Record<string, any> {
    id: Id
}
...
declare type ShortTagString = `${string}::${string}`
...
declare class AnyTrxNode {
    id: string
    globalId: string

    private children: AnyTrxNode[]

    private state?: 'open' | 'hold' | 'ok' | 'error'
    ...
    private scope: 'root' | `${string}::${string}:${string}` | `${string}::virtual`
    private trx: any
    private parent?: AnyTrxNode
    private module: any
    private auth?: {
        tokens: any,
        users: any
...
declare interface NesoiDuration {
    value: number
    unit: 'ms'|'milisecond'|'miliseconds'|'s'|'second'|'seconds'|'min'|'mins'|'minute'|'minutes'|'h'|'hour'|'hours'|'d'|'day'|'days'|'w'|'week'|'weeks'|'month'|'months'|'y'|'year'|'years'
    toString(): string
}
...
declare interface AuthProvider<A, B> {}

declare type AuthToken = string
declare type AuthRequest<P extends keyof any> = {
    [K in P]?: AuthToken
}

declare type AnyAuthnProviders = { [K: string]: AuthProvider<any, any> }
declare type AnyUsers = { [K: string]: User }
