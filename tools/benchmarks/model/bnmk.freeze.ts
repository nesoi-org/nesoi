...
function _freeze_meta(name: string) {
    if (name === 'very_simple') return () => use[0] = freeze_meta_simple(objs[0][1]);
    if (name === 'simple') return () => use[0] = freeze_meta_simple(objs[1][1]);
    if (name === 'complex') return () => use[0] = freeze_meta_complex(objs[2][1]);
    if (name === 'very_complex') return () => use[0] = freeze_meta_very_complex(objs[3][1]);
} 
