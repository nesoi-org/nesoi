...
export class BucketView<$ extends $BucketView> {
    ...
    public async parse<Obj extends NesoiObj>(
        ...
        const meta = await Daemon.getBucketReference(module.daemon!, tag);
    ...
    private async parseLayer(trx: AnyTrxNode, layer: ViewLayer, flags?: {
        ...
        for (const node of layer) {
            if (node.field.type !== 'model') continue;
            next.push(...this.parseModelProp(node, flags));
            
        }
        // Computed props
        for (const node of layer) {
            if (node.field.type !== 'computed') continue;
            await this.parseComputedProp(trx, node);
        }
        // Graph props
        for (const node of layer) {
            if (node.field.type !== 'graph') continue;
            next.push(...await this.parseGraphProp(trx, node));
        }
        // Drive props
        for (const node of layer) {
            if (node.field.type !== 'drive') continue;
            await this.parseDriveProp(trx, node);
        }
        // Group props
        for (const node of layer) {
            if (node.field.type !== 'group') continue;
    ...
    private async parseGraphProp(trx: AnyTrxNode, node: ViewNode) {
        ...
        const otherBucket = await Daemon.getBucketReference(daemon, otherBucketDep);
        ...
        if (node.field.children) {
            ...
            const module = TrxNode.getModule(trx);
            const subview_bucket = Daemon.getBucketReference(module.daemon!, link.bucket);
